...
void Position1( Robot *robot, double x, double y );// keyi ting 
//void shoot( Robot *robot, Environment *env );
double PredictBall1( Environment *env ,Robot *robot);
void shoumen(Robot *robot,Environment *env);
...
void PredictBall ( Environment *env )// yuan ben di
{
...
{double defendy;
	double dx = env->currentBall.pos.x - env->lastBall.pos.x;
	double dy = env->currentBall.pos.y - env->lastBall.pos.y;
	double time=(robot->pos.x-env->currentBall.pos.x)/dx;
	if(dy>0) // xiang zuo ti
	 defendy=env->lastBall.pos.y-dy*time;
	else if(dy<0)
 defendy=env->lastBall.pos.y+dy*time;
	else
 defendy=env->lastBall.pos.y;
	if(defendy>48.6)
		defendy=47.5;
	if(defendy<35.2)
		defendy=35.2;
...
{double defendy;
	PredictBall(env);
if(robot->pos .x <90 || robot->pos .x>93)  //zi shen x 
robot->pos .x =91.2;
	Position1(robot,robot->pos .x,env->predictedBall .pos .y );
	

if(env->predictedBall.pos.y>49.5)
Position1(robot,robot->pos .x ,48.6)
if(env->predictedBall.pos.y<35.2)
Position1(robot,robot->pos .x,35.2);
